...
1. Query MCP `@design-wiki/mcp`:
   - `semantic_search_components({ naturalLanguageQuery: "<intent>", targetDialProfile: { variance: 5 } })` (vector/dial-calibrated natural language search)
   - `compose_layout_tree({ pageType: "ai-chat-workspace" })` (synthesizes structured full-page layout scaffold)
