...
const REGISTRY_ITEMS: ComponentCardData[] = (registryData as any[]).map((item) => ({
  name: item.name,
  title: item.title || item.name,
  category: item.category || "ui:primitive",
  description: item.description || "Zero-slop component primitive for AI agents.",
  tags: Array.isArray(item.tags) ? item.tags : [],
  variance: item.dials?.design_variance ?? 3,
  motion: item.dials?.motion_intensity ?? 3,
  density: item.dials?.visual_density ?? 6,
}));

const CATALOG_SEED: ComponentCardData[] = REGISTRY_ITEMS;
...
export default function HomePage() {
  ...
  const categories = [
    ...
    { id: "ui:ai-native", label: "AI-Native" },
    { id: "ui:workflow", label: "Workflows" },
  ];
