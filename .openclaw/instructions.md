...
1. Query MCP `@design-wiki/mcp` (stdio: `npx @design-wiki/mcp`). Canonical tools:
   - `search_library({ query: "dock", category: "ui:motion" })` (alias: `search_components`) — filtered catalog search, <15KB payloads
   - `semantic_search_components({ naturalLanguageQuery: "<intent>", targetDialProfile: { variance: 5 } })` — keyword + dial scoring, not an embedding index
   - `compose_layout_tree({ pageType: "ai-chat-workspace" })` — page archetype scaffold from registry slugs
   - `fetch_raw_markup({ name: "floating-dock" })` (aliases: `fetch_raw_markdown`, `get_component_markup`)
   - `get_installation_schema({ name: "floating-dock", packageManager: "pnpm" })` (aliases: `get_installation_commands`, `get_install_recipe`)
   - `audit_code_slop({ code: "<raw-code>" })` / `audit_and_fix_slop({ code: "<raw-code>" })` — canonical `@design-wiki/audit-linter` rules
   - `get_dependency_graph({ name: "dock-magnification" })`
   - `verify_accessibility_contrast({ foregroundHex, backgroundHex })` — WCAG contrast math
2. Or load and parse `/public/llms.txt` and `/raw/components/<name>.md`. Counts live in `catalog-stats.json`.
